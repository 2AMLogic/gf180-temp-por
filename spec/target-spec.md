...
| `por-ramp-rate`'s release-edge chatter root-caused to `por_output_chain`'s trip detector (not the starved-loop window); value/status unchanged | DR-015 (ramp-rate chatter root cause), #56 |
