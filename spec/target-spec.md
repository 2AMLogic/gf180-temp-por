...
| <a id="por-hysteresis"></a>`por-hysteresis` | **V_hys** = VPOR↑ − VPOR↓ | **100 mV** | **150 mV [P]** | **250 mV [P]** | Both edges measured **at the same corner point** (hysteresis is a same-die difference, not a corner-to-corner one). **Measured (assembled path, full 81-point grid)**: **164.633–248.74 mV**, 81/81 within [100, 250] mV — min at `ff_125c_2.97v`, max at `ss_-40c_3.63v`. **Measured (Monte Carlo, local mismatch on, N = 500 at each of the five named binding points; both edges taken from one triangle ramp on the same die, so the figure stays a same-die difference under mismatch)**: σ = **0.77–0.97 mV**, mean ± 3σ spanning **150.48–166.51 mV**, **100 % empirical yield**, parametric 3σ inside [100, 250] mV at every point — **met, with ≈50 mV of margin above the 100 mV floor.** Mismatch is a far weaker term here than on the absolute thresholds because both edges share the same divider and the same comparator offset, which largely cancels in the difference. **Scope caveat (#61, [DR-013](decision-records/DR-013-por-brownout-spurious-assert.md)):** as with `por-vth-fall` above, this row's evidence is a continuous quasi-static ramp; the dip-shaped falling condition DR-013 measures produces a `VPOR↓` violation at up to 91 % of the grid at a faster (but still envelope-inside) falling rate, which correspondingly moves the *measured* hysteresis at those points — not absorbed into this row's own verdict, which stands on its own (different-topology) evidence. Evidence: [`sim/por-vth/records/20260801-233802-32fbaa0.md`](../sim/por-vth/records/20260801-233802-32fbaa0.md), [`sim/por-threshold-mc/records/20260802-083749-3b9b414.md`](../sim/por-threshold-mc/records/20260802-083749-3b9b414.md). **Post-layout finding (#85), root-caused and resolved (#187, [DR-021](decision-records/DR-021-por-hysteresis-quasi-static-scope.md)):** the composite/extracted full-assembly netlist (`layout/postlayout/temp_por_top.spice`, #82/PR #180) reproduces 80/81 grid points inside [100, 250] mV but the single worst-case corner, `ss_-40c_3.63v`, measures **261.092 mV** — 11.1 mV over the ceiling (vs. 248.74 mV on the schematic-level record above). **That reading is not one quantity.** `sim/por-vth/control/` re-measures the same corner across a 12.7–407.5 V/s ramp-rate ladder on both netlists and probes each comparator input against its static value: the deck traverses `vdd_val − 2.0 V` in a fixed 4 ms, so its **supply axis is also a ramp-rate axis** (242.5 / 325 / 407.5 V/s), and at 407.5 V/s the 261.092 mV decomposes as **143.3 mV of hysteresis** (the divider ratio, zero-rate limit — mid-window, and identical on the schematic netlist to within 0.05 mV), **97.7 mV of `VREF` being displaced by the moving rail** (+19.0 / −20.8 mV at the reference, × the divider ratio; ≈49 µs equivalent time constant, sign-reversing with ramp direction) and **20.1 mV of comparator/output-chain delay**. Held at one rate instead, `V_hys` spreads 4.3 mV rather than 49.7 mV over the whole ±10 % supply window and VPOR↑ is identical at all three supplies. **98 % of the +12.35 mV post-layout regression is the third term** — the extraction's 13.6–21.3 fF on the comparator's own internal nodes — not the divider: its drawn `SNS`/`SNSB` parasitics (25.9 / 17.5 fF, ~0.15 µs) are three orders too small, and its static hysteresis is unmoved. **Nothing is re-cost and no design change is made**: DR-021 leaves 100 / 150 / 250 mV untouched, scopes this row as the **quasi-static** value (measured `≈ V_hys,static + 0.32 mV/(V/s) · dVDD/dt` at the binding corner), and ratifies the `ss_-40c_3.63v` excursion as a measured, rate-scoped one rather than a defect. The ceiling's own rationale — keep VPOR↓,min ≥ 2.22 V — is independently satisfied and separately checked: [`por-vth-fall`](#por-vth-fall) passes 81/81 post-layout with VPOR↓,min = 2.37928 V, **159 mV above the floor**. The deck's supply/rate confound is a testbench defect, routed to #206; the `VREF` displacement to #208. Post-layout evidence: [`sim/por-vth/records/20260811-073945-12473c3.md`](../sim/por-vth/records/20260811-073945-12473c3.md) (overall FAIL, and it stands — `sim/` is append-only); root cause: [`sim/por-vth/control/results.md`](../sim/por-vth/control/results.md). | **[3σ]** · `ratifiable` (mismatch-inclusive; #15's MC closes the `conditional` tag; **quasi-static scope per DR-021**, and scoped to continuous-ramp falling rates per DR-013 — see both caveats; the post-layout 1/81 excursion is root-caused and ratified, not outstanding) |
...
|---|---|---|---|---|---|
| <a id="area"></a>`area` | Total block area (both sub-blocks, excluding pads) | **Measured (post-layout, assembled top cell's own bounding-box footprint) [DR-022]**: **1.059 mm² (1 059 196 µm²)**, `temp_por_top.gds` — wave-1 planning budget **≤0.05 mm² (≤50 000 µm²) [P]**, retained and recorded as **not met (21.2× over)** | The block's stated reason to exist is that "tiny area rides along on any shuttle seat"; that claim had no number anywhere until this measurement. Convention: the row means the assembled top cell's own footprint, not the unassembled sum of its four sub-cells' own boxes (0.342 mm², 6.8× over) or `klt stats`' drawn-polygon area (0.369 mm², not itself a footprint) — see [DR-022](decision-records/DR-022-area-post-layout-measurement.md) for why. Evidence: [`layout/reports/temp_por_top/stats.json`](../layout/reports/temp_por_top/stats.json), regenerated by `layout/run_checks.sh` (`klt stats`, #211) so the number cannot drift from the committed GDS. Target **not relaxed** — the planning bound stands, marked not met. | **[CWC]** · `pending #1` (measured, not met; disposition recorded per DR-022) | [DR-022](decision-records/DR-022-area-post-layout-measurement.md), `layout/reports/temp_por_top/stats.json` · #17, #211 |
...
| ~~[`area`](#area)~~ | ~~Measured post-layout area~~ — **filled by #211**: 1.059 mm² footprint (`temp_por_top.gds`), ≤0.05 mm² planning bound retained and recorded as **not met**, per [DR-022](decision-records/DR-022-area-post-layout-measurement.md) | ~~#17~~, #211 |
...
| `por-hysteresis` scoped as a **quasi-static** row, with its post-layout 1/81 excursion root-caused and ratified rather than left outstanding: 45 % of the failing corner's 261.092 mV reading is ramp-rate-induced — a 37 % `VREF` displacement plus an 8 % comparator/output delay term — and the divider ratio's own hysteresis is 143.3 mV, mid-window. No ratified value added, removed or relaxed, and no design change | DR-021 (por-hysteresis quasi-static scope), #187 |
| `area`'s `[TBD-#17]` placeholder replaced with the measured post-layout footprint (1.059 mm², `temp_por_top.gds`'s own bounding box, chosen over the 0.342 mm² tiled sub-cell sum and the 0.369 mm² drawn-polygon area as the row's convention); the ≤0.05 mm² wave-1 planning budget retained unchanged and recorded as **not met (21.2× over)**, not deleted or relaxed. Evidence wired into `layout/run_checks.sh` (`klt stats`) as regenerable `layout/reports/<cell>/stats.json` for all six cells | DR-022 (area post-layout measurement), #211 |
