...
**Every row's Evidence column is independently reproducible from this
repository**, though not every row is reproducible by `make characterize`
alone. Each cited `sim/<slug>/records/<record-id>.md` path names the
...
Five rows are disclosed exceptions to that single-command claim, each noted
at the row: the two `temp-accuracy-mc` rows and the two `por-iq` rows in
§4.1/§4.3 below need one additional, named manual command on top of
`make characterize`; §4.4's footprint row is layout evidence, not simulation
evidence, and is regenerated by `bash layout/run_checks.sh` instead. See the
repository [`README.md`](../../README.md)'s "Independent verification
(Chipalooza)" section for prerequisites, the three `make` targets
(`check`/`smoke`/`characterize`), measured wall-clock, and the exact
...
| Parameter | Min | Typ | Max | Absolute limit | Status @ 3.3 V rail | Status @ 5.0 V rail | Evidence |
|---|---|---|---|---|---|---|---|
| POR quiescent current (`RESETn` asserted, sensor disabled) | 0.656367 µA | — | 2.384647 µA | <3.0 µA (re-cost, [DR-018](../../spec/decision-records/DR-018-por-iq-recost.md)) | **Met** — 81/81 PASS, both netlist levels, 20.5 % margin at binding corner | Not yet characterized | `sim/por-iq/records/20260811-084152-68c0017-por-iq-derived.md` (post-layout), `…20260801-121458-660d016-por-iq-derived.md` (schematic) — `por-iq` has no `testbench/tb.json` and is not one of `make characterize`'s 22 discovered slugs; both files are regenerated by a manual derivation step against the `temp-accuracy-vt` record `make characterize` does mint, `python3 sim/por-iq/analyze_por_iq.py <temp-accuracy-vt-record-id> --write` |
...
- **One-command full characterization**: `make characterize` at the
  repository root reproduces every §4 row except the five disclosed
  exceptions noted in §4.1/§4.3/§4.4 (each needs one additional named,
  single-command manual step) from a clean clone (plus `make check`/`make
  smoke` for a fast environment/toolchain check and a quick end-to-end proof
  the flow runs) — see the repository
