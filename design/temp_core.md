...
- **Loop stability.** As of #270's re-run (table above), all four records are
  ...
  **#274 has since closed the remaining gap**:
  ...
  drawn `XCC` dimensions).

  **#298 has since re-run this same testbench against the extracted netlist**
  ([`layout/postlayout/temp_core.spice`](../layout/postlayout/temp_core.spice)),
  following the identical break-point construction translated to the
  extracted instance names: `XMP1`/`XMP2` are each fingered into two parallel
  unit devices by the layout (`X36`+`X47`, drain `M1D`; `X38`+`X45`, drain
  `M2D`), both move to the new `PG_FB` port, and `XMP3`'s split (`X40`+`X43`,
  drain `M3D`) is left on `PG`, exactly as above. `PG`'s own extracted
  interconnect (`RPG`/`CPG`, 5100.6 Ω / 61.7 fF — the amplifier's own output
  metal) stays on the `PG` side of the break. Record
  [`20260910-020408-c0e4d87`](../sim/temp-core-loop-stability/records/20260910-020408-c0e4d87.md)
  (`Supersedes: 20260819-182610-a4eebe7`) is 81/81 PASS, `dc_bias_delta_mv` = 0
  at every point exactly as the schematic record was, and measures:

  | Measurement | Min | Max |
  | --- | --- | --- |
  | Unity-gain crossover | 0.657 MHz (`res_ff_125c_2.97v`) | 1.442 MHz (`res_ss_-40c_3.63v`) |
  | Phase margin | **26.2°** (`ff_-40c_3.30v`) | **37.9°** (`ss_27c_3.63v`) |
  | Gain margin | **4.73 dB** (`ff_-40c_3.63v`) | **7.30 dB** (`res_ff_125c_2.97v`) |
  ...
  record (generated by `sim/postlayout_delta.py`, not hand-written) shows
  **zero regressions**: both checked measurements (`dc_bias_delta_mv`,
  `phase_margin_deg`) stay `ok -> ok`. The extracted parasitics do measurably
  cost margin — the worst-case phase margin drops from the schematic's 34.2°
  to 26.2° (a corner-matched −9.3° at `ss_-40c_3.63v`, the delta record's
  worst-case row), and the crossover frequency and `fpx0` both fall ~12–15 %
  at their worst corners — consistent with the extra ~62 fF / 5.1 kΩ `RPG`/
  `CPG` now loading the compensation node that the schematic-level record
  measured without. Gain margin actually improves slightly at its own
  worst corner (the extra pole moves where the loop phase crosses zero, not
  just how fast it gets there). None of this trips a ratified bound — see
  below — but it is the number a reviewer asking "how much margin does the
  extracted netlist actually leave" should cite from now on, not the
  schematic-level one.
  ...
  **No target bound is ratified against these numbers.** Per #274's own
  ...
  this record, the extracted re-run above, or the transient records
  elsewhere in this document shows evidence of an actual instability
  problem; the worst-case 26.2° extracted margin is measured at the
