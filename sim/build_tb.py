...
def _assemble(
    ...
    """Shared body of :func:`build` and :func:`build_postlayout`: read the
    stimulus fragment and each DUT cell netlist under ``source_dir``, and
    stitch them into one GENERATED fragment with a provenance header (each
    source's path -- prefixed with ``source_label`` -- and sha256).
    """
    ...
    bodies = []
    for cell in cells:
        text = (source_dir / f"{cell}.spice").read_text()
        digest = hashlib.sha256(text.encode()).hexdigest()
        header.append(f"*   {source_label}/{cell}.spice  (sha256 {digest})")
        bodies.append(text)
...
def build(slug: str) -> str:
    return _assemble(slug, FRAGMENTS, NETLIST_DIR, "design/netlist")
...
def build_postlayout(slug: str) -> str:
    ...
    """
    return _assemble(slug, POSTLAYOUT_FRAGMENTS, POSTLAYOUT_DIR, "layout/postlayout")
